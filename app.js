...
// 5b. WELCOME SPLASH SCREEN (Shows only once per session / visit)
function initWelcomeSplash() {
  const splash = document.getElementById('welcomeSplash');
  if (!splash) return;

  const hasSeenSplash = sessionStorage.getItem('dtl_welcome_seen') || localStorage.getItem('dtl_welcome_seen');
  if (hasSeenSplash) {
  ...
  setTimeout(() => {
    ...
    document.body.style.overflow = '';
    sessionStorage.setItem('dtl_welcome_seen', 'true');
    localStorage.setItem('dtl_welcome_seen', 'true');
    setTimeout(() => {
...
// 8. TOP SELLING - Clean simple cards: image, heart button, name, rating, ONLY BUY NOW button
    ...
    return `
      <div class="product-card ts-card" id="card-${product.id}">
        ...
        <div class="product-content ts-content">
          ...
          <div class="product-card-actions ts-card-actions">
            <button class="btn btn-red buy-now-btn ts-buy-btn" onclick="${product.isHoney ? "window.location.href='honey.html'" : `openProductSelectionModal('${targetProdId}')`}">
...
// 9. ALL PRODUCTS - Top-left view icon, Top-right favorite, Cart Icon + Buy Now
    ...
    html += `
      <div class="product-card" id="allcard-${product.id}">
        <div class="product-img-box">
          <!-- Top-Left: View Icon Only -->
          <button class="card-view-btn" onclick="openProductQuickView('${product.id}', event)" title="Quick View">
        ...
        <div class="product-content">
          ...
          <div class="product-card-actions ap-card-actions">
            <button class="btn btn-cart-icon" onclick="quickAddToCart('${product.id}', event)" title="Add to Cart">
...
// 9a. QUICK ADD TO CART DIRECTLY FROM PRODUCT CARD ICON
function quickAddToCart(productId, event) {
  ...
  const variant = (product.variants && product.variants.length > 0)
    ? (product.variants[product.selectedWeightIndex || 0] || product.variants[0])
    : (product.defaultVariant || { weight: 'Standard', price: 1000 });
...
function proceedDirectOrderFromModal() {
  if (!requireLoginForOrder('Please create an account or login first to place an order.')) return;
  if (!currentModalProduct) return;
  const product = currentModalProduct;
  const variant = product.variants[modalSelectedVariantIndex] || product.variants[0];
  const itemId = `${product.id}-${variant.weight.replace(/\s+/g,'')}`;
...
// 12. AUTH & LOGIN SYSTEM
function requireLoginForOrder(actionMessage = 'Please login or create an account first to place your order.') {
  if (!currentUser) {
    showToast(`<i class="fa-solid fa-user-lock text-red"></i> ${actionMessage}`);
    closeCartDrawer();
    closeCheckoutModal();
    closeProductSelectionModal();
    ...
    if (authSec) {
      ...
      registeredUsers = JSON.parse(localStorage.getItem('dtl_registered_users')) || [];
      if (registeredUsers.length === 0 && signupTabBtn && signupForm) {
...
function updateAuthUI() {
  ...
  if (currentUser) {
    ...
    if (loggedState) {
      ...
      if (emailEl) emailEl.textContent = currentUser.email + (currentUser.phone ? ` • ${currentUser.phone}` : '');
    }
  } else {
    if (loggedState) loggedState.style.display = 'none';
    if (authTabs) authTabs.style.display = 'flex';
    const signupTabBtn = document.getElementById('signupTabBtn');
    if (signupTabBtn && signupTabBtn.classList.contains('active')) {
...
function openProductQuickView(productId, event) {
  if (event) {
    event.stopPropagation();
    event.preventDefault();
  }
...
function openCheckoutModal() {
  if (!requireLoginForOrder('Please login or create an account before proceeding to checkout.')) return;
  if (cart.length === 0) { showToast('Cart is empty!'); return; }
  closeCartDrawer();

  // Auto-fill customer details from currentUser
  const nameInput = document.getElementById('custName');
  const phoneInput = document.getElementById('custPhone');
  if (nameInput && currentUser && currentUser.name && !nameInput.value) {
    nameInput.value = currentUser.name;
  }
  if (phoneInput && currentUser && currentUser.phone && !phoneInput.value) {
...
function initEventListeners() {
  ...
  // Login handler with verification against registered users
  loginForm?.addEventListener('submit', (e) => {
    e.preventDefault();
    const loginIdentifier = document.getElementById('loginEmail').value.trim().toLowerCase();
    ...
    if (!matchedUser) {
      const userExists = registeredUsers.some(u => u.email.toLowerCase() === loginIdentifier || u.phone === loginIdentifier);
      if (userExists) {
        showToast('<i class="fa-solid fa-triangle-exclamation text-red"></i> Incorrect password. Please check and try again.');
      } else {
        showToast('<i class="fa-solid fa-triangle-exclamation text-red"></i> No account found with this email/phone. Please create an account first.');
        if (signupTabBtn && signupForm) {
          signupTabBtn.classList.add('active');
          loginTabBtn?.classList.remove('active');
          signupForm.style.display = 'flex';
          if (loginForm) loginForm.style.display = 'none';
          const signupEmail = document.getElementById('signupEmail');
          if (signupEmail && loginIdentifier.includes('@')) signupEmail.value = loginIdentifier;
          const signupPhone = document.getElementById('signupPhone');
          if (signupPhone && !loginIdentifier.includes('@')) signupPhone.value = loginIdentifier;
        }
      }
      return;
    }
    ...
    updateAuthUI();
    showToast(`<i class="fa-solid fa-circle-check text-red"></i> Welcome back, <strong>${currentUser.name}</strong>!`);
  });

  // Signup handler with validation and storage
  signupForm?.addEventListener('submit', (e) => {
    ...
    const alreadyExists = registeredUsers.some(u => u.email.toLowerCase() === email || u.phone === phone);
    if (alreadyExists) {
      showToast('<i class="fa-solid fa-triangle-exclamation text-red"></i> An account with this email/phone already exists. Please login.');
    ...
    const newUser = { name, phone, email, password, createdAt: Date.now() };
    ...
    localStorage.setItem('dtl_user', JSON.stringify(currentUser));
    updateAuthUI();
    showToast(`<i class="fa-solid fa-circle-check text-red"></i> Account created successfully! Welcome, <strong>${name}</strong>!`);
  });
  ...
  document.getElementById('checkoutForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    if (!requireLoginForOrder('Please login or create an account first to place your order.')) return;
    ...
    fetch('https://formsubmit.co/ajax/zaibbabar54@gmail.com', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ _subject: `New Order ${refId} - DESI TASTE LAND`, Order_Reference: refId, Customer_Name: name, Customer_Phone: phone, Customer_City: city, Shipping_Address: address, Items_Ordered: itemsSummary, Total_Amount_PKR: grandTotal, Account_Email: currentUser ? currentUser.email : '' }) }).catch(() => {});
